...
import {useState, useRef, Component, onMounted, onWillUnmount, useEffect} from "@odoo/owl";
...
export class CameraDialog extends Component {
    ...
    _confirm() {
        ...
        canvas.toBlob((blob) => {
            ...
            const file = new File([blob], performance.now() + ".jpg", {type: "image/jpeg"});
    ...
    get httpParams() {
        ...
        const params = {
            csrf_token: odoo.csrf_token, ufile: [this.state.file],
        };
        if (resModel) {
...
CameraDialog.template = "vitech.camera_dialog";
CameraDialog.components = {Dialog};
CameraDialog.defaultProps = {
    title: "Máy ảnh", route: "/web/binary/upload_attachment", onUpload: () => {
    },
};